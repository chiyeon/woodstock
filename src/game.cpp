...
void Game::read_fen(std::string fen) {
  ...
  Piece all_pieces[12] = {
      Pieces::PAWN | Pieces::WHITE,   Pieces::KNIGHT | Pieces::WHITE,
      Pieces::BISHOP | Pieces::WHITE, Pieces::ROOK | Pieces::WHITE,
      Pieces::QUEEN | Pieces::WHITE,  Pieces::KING | Pieces::WHITE,
      Pieces::PAWN | Pieces::BLACK,   Pieces::KNIGHT | Pieces::BLACK,
      Pieces::BISHOP | Pieces::BLACK, Pieces::ROOK | Pieces::BLACK,
      Pieces::QUEEN | Pieces::BLACK,  Pieces::KING | Pieces::BLACK,
  };

  for (int i = 0; i < 12; ++i) {
    piece_bbs[all_pieces[i]] =
        Bitboards::board_to_bitboard(board, Pieces::NO_FILTER, all_pieces[i]);
  }
  piece_bbs[Pieces::WHITE] =
      Bitboards::board_to_bitboard(board, Pieces::FILTER_COLOR, Pieces::WHITE);
...
Bitboard Game::get_game_bitboard() { return game_bb; }

Bitboard Game::get_piece_bb(Piece filter) { return piece_bbs[filter]; }

Piece Game::get(int index) { return board[index]; }

Piece Game::get(int x, int y) { return board[y * 8 + x]; }

History &Game::get_history() { return history; }

bool Game::is_history_empty() { return history.is_empty(); }

Move Game::get_last_move() { return history.get_last_move(); }

std::vector<Move> Game::get_moves_at_square(int sq) {
  std::vector<Move> out_moves;

  if (get(sq) == 0)
    return out_moves; // empty
  if ((get(sq) & Pieces::FILTER_COLOR) != turn)
    return out_moves; // other team
...
bool Game::is_blacks_turn() { return turn == Pieces::BLACK; }

bool Game::is_whites_turn() { return turn == Pieces::WHITE; }
...
Piece Game::get_turn() { return turn; }
...
bool Game::last_move_resulted_in_check() {
  ...
  if (axis_bitboard == 0ULL)
    return false; // early escape
  if (king_position == 0ULL)
    return false; // if kind doesnt exist (testing) just return false
...
bool Game::is_king_in_check() {
  ...
  return (piece_bbs[not_turn | Pieces::QUEEN] &
          ...
          piece_bbs[not_turn | Pieces::ROOK] &
              Pieces::get_rook_moves(king_pos, *this) ||
          piece_bbs[not_turn | Pieces::BISHOP] &
              Pieces::get_bishop_moves(king_pos, *this) ||
...
bool Game::no_moves_left() {
  ...
  Bitboard attacks_on_king = 0ULL; // pieces + attacks against the king. These
                                   // spaces can be blocked/captured to stop it
  int num_king_attackers = 0;      // determine if we cannot block a check

  Bitboard blocker_no_king =
      game_bb & ~king_position; // so we can "x-ray" the king. represents the
                                // game bitboard with the active king removed
  ...
  while (axis_bitboard_i != 0ULL) {
    ...
    case Pieces::QUEEN:
      piece_captures = movemasks.get_rook_move(blocker_no_king, pos) |
                       movemasks.get_bishop_move(blocker_no_king, pos);
      if (piece_captures & king_position) {
        attacks_on_king |= Bitboards::get_between(king_x, king_y, pos_x, pos_y);
        ++num_king_attackers;
      }
  ...
  std::vector<Bitboard>
      pinned_spaces; // if any of our pieces are in one of these spaces, they
                     // can only move in that area!
  if (num_king_attackers < 2) {
    Bitboard potential_pinners =
        Pieces::get_queen_moves_using_blocker(king_x + king_y * 8,
                                              axis_bitboard, *this) &
        axis_bitboard;
    ...
    while (potential_pinners != 0ULL) {
      ...
      // if so, that ally is restricted to that area (non inclusive of ally,
      // inclusive of axis pieces)

      int pos_x = pos % 8;
      int pos_y = pos / 8;

      // calculate the in between
      Bitboard between = Bitboards::get_between(
          king_x, king_y, pos_x, pos_y, (board[pos] & Pieces::FILTER_PIECE));
  ...
  while (ally_bitboard_i != 0ULL) {
    ...
    if (num_king_attackers == 2 &&
        (piece & Pieces::FILTER_PIECE) != Pieces::KING)
      continue;
    ...
    case Pieces::PAWN: {
      ...
      // piece_moves =
      // Pieces::get_pawn_forward<black_turn>(Bitboards::get_i(pos));
      ...
      if ((piece_moves & Bitboards::ROW_1) != 0 ||
        ...
        while (pm_i != 0ULL) {
          ...
          for (int i = 0; i < 4; ++i) {
            Move potential_move = Moves::create(pos, target_pos, pieces[i],
                                                captured, Moves::PROMOTION);
      ...
      if (
        ...
        Move potential_move =
            Moves::create(pos, target_pos, piece, Moves::get_piece(last_move),
                          Moves::EN_PASSANT);
        move(potential_move);
        if (!last_move_resulted_in_check()) {
          piece_moves |= Bitboards::get_i(target_pos);
    ...
    // if we are in check by 1 attacker, we can kill it, block it, or move the
    // king
...
bool Game::can_castle_kingside(Bitboard attacked_squares) {
  if (is_blacks_turn()) {
    ...
    if (Bitboards::BLACK_KINGSIDE_CASTLE_SPACES & game_bb)
      return false;
    if (Bitboards::BLACK_KINGSIDE_CASTLE_ATTACK_FREE & attacked_squares)
      return false;
    ...
    if (Bitboards::WHITE_KINGSIDE_CASTLE_SPACES & game_bb)
      return false;
    if (Bitboards::WHITE_KINGSIDE_CASTLE_ATTACK_FREE & attacked_squares)
      return false;
...
bool Game::can_castle_queenside(Bitboard attacked_squares) {
  if (is_blacks_turn()) {
    ...
    if (Bitboards::BLACK_QUEENSIDE_CASTLE_SPACES & game_bb)
      return false;
    if (Bitboards::BLACK_QUEENSIDE_CASTLE_ATTACK_FREE & attacked_squares)
      return false;
...
void Game::move(Move &move, bool verbose) {
  ...
  case Moves::EN_PASSANT: {
    int ep_square =
        is_blacks_turn() ? Moves::get_to(move) + 8 : Moves::get_to(move) - 8;
  ...
  case Moves::CASTLE: {
    ...
    if (to > from) { // queenside
      ...
      is_blacks_turn() ? has_black_queenside_rook_moved = true
                       : has_white_queenside_rook_moved = true;
    } else { // kingside
      Piece rook = Pieces::ROOK | turn;
...
void Game::undo(bool verbose) {
  ...
  while (flags != 0) {
    ...
    case Moves::FIRST_MOVE: {
      switch (piece & Pieces::FILTER_PIECE) {
      case Pieces::KING:
        is_blacks_turn() ? has_black_king_moved = false
                         : has_white_king_moved = false;
        break;
      case Pieces::ROOK:
        if (is_blacks_turn()) {
