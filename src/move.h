...
struct Moves
{
    ...
    static constexpr Flag FIRST_MOVE        =   0b100;   // not assigned until the move is made. used for undo()
    ...
    static constexpr Move FILTER_PIECE      =           0b11111000000000000;    // num 0-22 for pieces
    ...
    static inline Move create(int from = 0, int to = 0, int piece = 0, int captured = 0, Flag flags = 0) {
        return
            from | 
            to << SHIFT_TO |
            piece << SHIFT_PIECE |
            captured << SHIFT_CAPTURED |
            flags << SHIFT_FLAGS;
    }

    static inline int get_from(const Move & move) {
        return move & FILTER_FROM;
    }
