 ...
 * used for blocking, does NOT include the last square in any particular
 * direction
 */
  ...
  for (int y = 0; y < 8; ++y) {
    for (int x = 0; x < 8; ++x) {
      ...
      rook_magic_table[i].mask =
          Bitboards::get_row(y) ^ Bitboards::get_column(x);

      if (x != 0)
        rook_magic_table[i].mask &= ~Bitboards::get_column(0);
      if (x != 7)
        rook_magic_table[i].mask &= ~Bitboards::get_column(7);
      if (y != 0)
        rook_magic_table[i].mask &= ~Bitboards::get_row(0);
      if (y != 7)
        rook_magic_table[i].mask &= ~Bitboards::get_row(7);
    }
  }
}
...
void MoveMasks::initialize_magic_table() {
  int rook_shift_bits[64] = {12, 11, 11, 11, 11, 11, 11, 12, 11, 10, 10, 10, 10,
  ...
  int bishop_shift_bits[64] = {6, 5, 5, 5, 5, 5, 5, 6, 5, 5, 5, 5, 5, 5, 5, 5,
  ...
  for (int i = 0; i < 64; ++i) {
    rook_magic_table[i] =
        MagicEntry{nullptr, 0ULL, 0ULL, 64 - rook_shift_bits[i]};
    bishop_magic_table[i] =
        MagicEntry{nullptr, 0ULL, 0ULL, 64 - bishop_shift_bits[i]};
  }
}

Bitboard MoveMasks::get_knight_move(int pos) { return knight_moves[pos]; }

Bitboard MoveMasks::get_king_move(int pos) { return king_moves[pos]; }
...
// given a bishop/rook blocker mask, return all possible combinations of
// blockers that can be in that mask
std::vector<Bitboard>
MoveMasks::get_all_blocker_combinations(Bitboard blocker) {
  std::vector<Bitboard> blockers_combinations;
  // todo reserve
  blockers_combinations.push_back(blocker); // add fully blocked as one

  Bitboard count_board =
      0ULL; // this will increment like binary does each iteration
...
Magic MoveMasks::find_magic(int pos, Piece piece_type) {
   ...
   * for that mask
   *
   * then for each blocker combination calculate the proper positions with
   * raycasting then, use that to determine the magics by brute forcing
   *
  ...
  // printf("Finding magics for %s\n", piece_type == Pieces::BISHOP ? "bishops"
  // : "rooks");

  // Bitboard start_position = Bitboards::get_i(pos);
  Bitboard mask = (piece_type == Pieces::BISHOP) ? bishop_magic_table[pos].mask
                                                 : rook_magic_table[pos].mask;
  std::vector<Bitboard> blocker_combinations =
      get_all_blocker_combinations(mask);
  ...
  std::vector<Bitboard> magic_moves(
      num_magic_moves,
      0xFFFFFFFFFFFFFFFFULL); // this number is impossible for normal movement,
                              // thus it becomes our "empty" state
  int index_bits = (piece_type == Pieces::BISHOP)
                       ? bishop_magic_table[pos].shift
                       : rook_magic_table[pos].shift;
  ...
  for (int i = 0; i < num_moves; ++i) {
    ...
    if (piece_type == Pieces::BISHOP) {
      for (int j = 1; (x + j < 8) && (y + j < 8); ++j) {
        ...
        if (blocker_combinations[i] & target_square)
          break;
      }
  ...
  for (int i = 0; i < num_tries; ++i) {
    ...
    fill(magic_moves.begin(), magic_moves.end(),
         0xFFFFFFFFFFFFFFFFULL); // reset magic moves
    for (int j = 0; j < num_moves; ++j) {
      Bitboard key = (blocker_combinations[j] * magic) >> index_bits;
      if (key >= (Bitboard)num_magic_moves)
        goto next_magic;
      if (magic_moves[key] == 0xFFFFFFFFFFFFFFFF) {
        magic_moves[key] = legal_moves[j];
        // element_count++;
      } else if (magic_moves[key] != legal_moves[j]) {
        // not a good magic if there is a collision that ISNT the same legal
        // move
        goto next_magic;
      }
    }
    ...
    if (piece_type == Pieces::BISHOP) {
      ...
      for (int j = 0; j < magic_moves_size; ++j) {
        if (magic_moves[j] != 0xFFFFFFFFFFFFFFFFULL)
